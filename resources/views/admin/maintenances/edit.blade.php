...
<x-layouts.app>
    <div class="flex h-full w-full flex-1 flex-col gap-6 p-6 lg:p-8">
        ...
        <div class="flex items-center justify-between">
            <div>
                <flux:heading size="xl">Editar Mantenimiento</flux:heading>
                <flux:subheading>Actualiza la información del mantenimiento</flux:subheading>
            </div>
        ...
        <form method="POST" action="{{ route('admin.maintenances.update', $maintenance) }}" class="space-y-6">
            @csrf
            @method('PUT')
                    ...
                    <div>
                        <flux:select name="vehiculo_id" label="Vehículo" placeholder="Selecciona un vehículo" required>
                            @foreach($vehicles as $vehicle)
                                <option value="{{ $vehicle->id }}" {{ old('vehiculo_id', $maintenance->vehiculo_id) == $vehicle->id ? 'selected' : '' }}>
                    ...
                    <div>
                        <flux:select name="tipo" label="Tipo de Mantenimiento" placeholder="Selecciona tipo" required>
                            @foreach(MaintenanceType::cases() as $tipo)
                                <option value="{{ $tipo->value }}" {{ old('tipo', $maintenance->tipo->value) === $tipo->value ? 'selected' : '' }}>
                                    {{ $tipo->label() }}
                                </option>
                            @endforeach
                        </flux:select>
                    ...
                    <div>
                        <flux:select name="estado" label="Estado" placeholder="Selecciona estado">
                            @foreach(MaintenanceStatus::cases() as $estado)
                                <option value="{{ $estado->value }}" {{ old('estado', $maintenance->estado->value) === $estado->value ? 'selected' : '' }}>
                                    {{ $estado->label() }}
                                </option>
                            @endforeach
                        </flux:select>
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    <div>
                        <flux:input 
                            ...
                            value="{{ old('fecha_programada', $maintenance->fecha_programada?->format('Y-m-d')) }}"
                        />
                    ...
                    <div>
                        <flux:input 
                            ...
                            value="{{ old('fecha_realizada', $maintenance->fecha_realizada?->format('Y-m-d')) }}"
                        />
                ...
                <div class="grid gap-6 md:grid-cols-3">
                    <div>
                        <flux:input 
                            ...
                            placeholder="0"
                            value="{{ old('kilometraje_actual', $maintenance->kilometraje_actual) }}"
                            min="0"
                        />
                    ...
                    <div>
                        <flux:input 
                            ...
                            value="{{ old('kilometraje_proximo', $maintenance->kilometraje_proximo) }}"
                            min="0"
                        />
                    ...
                    <div>
                        <flux:input 
                            ...
                            label="Intervalo (km)" 
                            placeholder="Ej: 5000"
                            value="{{ old('kilometraje_intervalo', $maintenance->kilometraje_intervalo) }}"
                            min="0"
                        />
                ...
                <div class="space-y-4">
                    <div>
                        <flux:textarea 
                            ...
                            rows="4"
                            required
                        >{{ old('descripcion', $maintenance->descripcion) }}</flux:textarea>
                    ...
                    <div class="grid gap-6 md:grid-cols-2">
                        <div>
                            <flux:input 
                                ...
                                value="{{ old('taller', $maintenance->taller) }}"
                            />
                        ...
                        <div>
                            <flux:input 
                                ...
                                value="{{ old('mecanico', $maintenance->mecanico) }}"
                            />
                    ...
                    <div>
                        <flux:textarea 
                            ...
                            rows="3"
                        >{{ old('repuestos_usados', $maintenance->repuestos_usados) }}</flux:textarea>
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    <div>
                        <flux:input 
                            ...
                            placeholder="0"
                            value="{{ old('costo', $maintenance->costo) }}"
                            min="0"
                            step="0.01"
                        />
                    ...
                    <div>
                        <flux:textarea 
                            ...
                            rows="3"
                        >{{ old('observaciones', $maintenance->observaciones) }}</flux:textarea>
