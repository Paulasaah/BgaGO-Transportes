...
<footer class="relative bg-white dark:bg-black border-t border-zinc-200 dark:border-white/10">
    <div class="pt-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid md:grid-cols-4 gap-12">
        <!-- Logo and Description -->
        <div>
            <div class="flex items-center space-x-3 mb-4">
                <div class="w-10 h-10 rounded-full bg-blue-600 dark:bg-blue-500 flex items-center justify-center">
                    <x-app-logo-icon class="w-6 h-6 text-white" />
                </div>
                <span class="text-2xl font-bold text-blue-600 dark:text-blue-400">BgaGO</span>
            </div>
            ...
            <!-- Redes Sociales -->
            <div class="flex space-x-4 mt-6 text-zinc-500 dark:text-zinc-400">
        ...
        <!-- Links -->
        <div>
            <h3 class="text-lg font-semibold mb-4 text-blue-600 dark:text-blue-400">Enlaces</h3>
            <ul class="space-y-2 text-zinc-600 dark:text-zinc-400 text-sm">
                <li><a href="{{ auth()->check() ? ((auth()->user()->isAdmin() || auth()->user()->isDriver()) ? route('dashboard') : route('user.home')) : (\Illuminate\Support\Facades\Route::has('home') ? route('home') : '/') }}" class="hover:text-blue-500 transition-colors" wire:navigate>Inicio</a></li>
                <li><a href="{{ route('services.delivery') }}" class="hover:text-blue-500 transition-colors" wire:navigate>Servicios</a></li>
                <li><a href="{{ \Illuminate\Support\Facades\Route::has('mapa') ? route('mapa') : '/mapa' }}" class="hover:text-blue-500 transition-colors">Mapa</a></li>
                <li><a href="{{ route('catalog.index') }}" class="hover:text-blue-500 transition-colors" wire:navigate>Catálogo</a></li>
                <li><a href="{{ \Illuminate\Support\Facades\Route::has('login') ? route('login') : '/login' }}" class="hover:text-blue-500 transition-colors">Iniciar sesión</a></li>
            </ul>
        </div>

        <!-- Contact Info -->
        <div>
            <h3 class="text-lg font-semibold mb-4 text-blue-600 dark:text-blue-400">Contáctanos</h3>
        ...
        <!-- Newsletter -->
        <div>
            <h3 class="text-lg font-semibold mb-4 text-blue-600 dark:text-blue-400">Suscríbete</h3>
    ...
    <div class="relative z-10 mt-16 border-t border-zinc-200 dark:border-white/10 pt-6 text-center text-sm">
        <p class="text-zinc-600 dark:text-zinc-400">&copy; {{ date('Y') }} <span class="font-semibold text-zinc-900 dark:text-white">BgaGO</span>. Todos los derechos reservados.</p>
        <p class="mt-1 text-zinc-500 dark:text-zinc-400">Diseñado con enfoque empresarial</p>
    </div>
</footer>
