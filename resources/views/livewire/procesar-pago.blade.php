...
<div>
    ...
    <form wire:submit.prevent="procesarPago" class="space-y-6">
        ...
        @elseif($metodo_seleccionado === 'wallet')
            <div class="space-y-6">
                <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 bg-gradient-to-r from-zinc-50 to-white dark:from-zinc-900 dark:to-zinc-800 p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <div class="text-3xl font-bold text-zinc-900 dark:text-white">COP 0</div>
                        </div>
                    </div>
                </div>

                <div>
                    <h4 class="text-base font-bold text-zinc-900 dark:text-white mb-3">Métodos de Pago</h4>
                    <div class="grid md:grid-cols-2 gap-4">
                        @forelse($wallet_cards as $i => $card)
                            <button type="button" wire:click="walletUseCard({{ $i }})" class="text-left p-5 rounded-xl bg-blue-900 text-white hover:bg-blue-800 transition-colors">
                                <div class="flex items-center justify-between">
                                    <span class="text-sm">{{ strtoupper(($card['brand'] ?? 'Visa')) }}</span>
                                    <span class="px-2 py-1 text-[10px] rounded bg-white text-blue-900 font-bold">{{ strtoupper($card['tipo'] ?? 'credito') }}</span>
                                </div>
                                <div class="mt-4 tracking-widest">**** {{ substr(preg_replace('/\D/', '', ($card['numero'] ?? '')), -4) }}</div>
                                <div class="mt-2 text-sm opacity-80">{{ $card['nombre'] ?? '' }}</div>
                            </button>
                        @empty
                            <div class="text-sm text-zinc-600 dark:text-zinc-400">No tienes tarjetas guardadas. Agrega una en tu Wallet.</div>
                        @endforelse
                    </div>
                </div>
            </div>
        ...
        <div class="space-y-4">
            <label class="flex items-start gap-3">
                <input type="checkbox" wire:model.live="acepta_terminos" class="mt-1 h-5 w-5 rounded border-zinc-300 dark:border-zinc-600 text-blue-600 focus:ring-blue-500">
                <span class="text-sm text-zinc-700 dark:text-zinc-300">Acepto los términos, condiciones y políticas de privacidad</span>
            </label>
